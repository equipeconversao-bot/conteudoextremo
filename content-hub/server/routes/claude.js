...
async function callClaude(systemPrompt, userMessage, maxTokens = 800) {
  const key = getApiKey()
  if (!key) {
    return { error: 'ANTHROPIC_API_KEY não configurada. Adicione no .env: ANTHROPIC_API_KEY=sua-chave' }
  }
  ...
  if (!res.ok) {
    ...
    return { error: `Claude error: ${res.status} — ${err}` }
  }
...
claudeRouter.post('/chat', async (req, res) => {
...
${context ? JSON.stringify(context, null, 2) : 'Nenhum contexto específico'}`
  ...
  const result = await callClaude(systemPrompt, message, 1200)
  res.json(result)
})

claudeRouter.post('/ideias-carrossel', async (req, res) => {
  const { tema, nicho } = req.body
...
claudeRouter.post('/roteiro-reels', async (req, res) => {
...
claudeRouter.post('/roteiro-youtube', async (req, res) => {
  const { tema, duracao } = req.body
