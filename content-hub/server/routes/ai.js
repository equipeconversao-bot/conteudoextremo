...
async function callClaude(messages, systemPrompt, customApiKey = '') {
...
async function callOpenAI(messages, systemPrompt, customApiKey = '') {
...
aiRouter.post('/generate', async (req, res) => {
  const { type, context, userPrompt, apiKey, messages } = req.body
  ...
  let result = await callClaude(formattedMessages, system, apiKey)

  // 2. Fallback to OpenAI if Claude fails or no key
  if (!result) {
    result = await callOpenAI(formattedMessages, system, apiKey)
  }

  // 3. Smart simulation fallback if no API keys are configured
  if (!result) {
